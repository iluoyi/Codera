def _shortest(tree, x, y):
    ...
    if rdep_x >=0 and rdep_y >=0: 
        return rdep_x, rdep_y
    return (max(ldep_x, rdep_x) + 1 if ldep_x * rdep_x <=0 else \
            0 if tree is x else -1,
            max(ldep_y, rdep_y) + 1 if ldep_y * rdep_y <=0 else \
            0 if tree is y else -1)
...
print shortest(t, t.right.left, t.right.left)  # x == y
